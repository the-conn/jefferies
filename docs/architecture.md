...
* **KubeDispatcher:** The live `Dispatcher` implementation that actuates each pipeline node as a Kubernetes Job. There is a single node template — every node, regardless of workload, runs under the same shape:
  ...
  * **Run metadata env vars:** every node Pod is injected with a fixed set of `JEFFERIES_*` environment variables derived from the run's `RunContext` so user steps can branch on what triggered the run: `JEFFERIES_RUN_ID`, `JEFFERIES_TRIGGER` (`push` / `pull_request` / `retry`), `JEFFERIES_OWNER`, `JEFFERIES_REPO`, `JEFFERIES_SHA`, `JEFFERIES_BRANCH`, `JEFFERIES_TARGET_BRANCH`, `JEFFERIES_TAG`, `JEFFERIES_PR_NUMBER`. Every variable is always set; values that don't apply to the current trigger render as the empty string so user scripts can rely on the variable being defined. The user's `env:` map is layered on top of this set, so a node-level `env` entry can override any of them on a per-node basis.
