...
* **KubeDispatcher:** The live `Dispatcher` implementation that actuates each pipeline node as a Kubernetes Job. There is a single node template — every node, regardless of workload, runs under the same shape:
  ...
  * **Vault secret injection:** when a node has any `secrets:` declared (top-level pipeline secrets are merged into each node), the Pod template is decorated with Vault Agent Injector annotations: `vault.hashicorp.com/agent-inject`, `agent-pre-populate-only`, `secret-volume-path: /etc/tube/secrets`, `client-max-retries: "0"` (so a missing primary path falls through to the fallback immediately instead of looping), `role`, plus a `(agent-inject-secret-<KEY>, agent-inject-template-<KEY>)` pair per secret. Each template tries `secret/data/<owner>/<repo>` first and falls back to `secret/data/<owner>`, letting org-level secrets be overridden per-repo. Rendered files land in tmpfs at `/etc/tube/secrets/<KEY>`.
  * The node image, Tube image, target namespace, privileged + default service accounts, Vault role, runtime class, and default CPU/memory are all user-configurable in `[kubernetes]` of the TOML config.
