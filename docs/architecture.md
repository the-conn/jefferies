...
* **KubeDispatcher:** The live `Dispatcher` implementation that actuates each pipeline node as a Kubernetes Job. There is a single node template — every node, regardless of workload, runs under the same shape:
  * **Conditional sandboxing on `privileged`:** non-privileged nodes run on the cluster's default runtime under the namespace's default ServiceAccount, which keeps IO-heavy workloads (Rust compilation, `cargo test`, etc.) on the fast path and avoids the kata performance overhead. Privileged nodes (`privileged: true`) are submitted with `runtimeClassName: kata` and `serviceAccountName: jefferies-jobs-sa` so VM-level isolation is in place before any host-breaking primitive (overlay mounts, `--network=host`, `buildah bud`) is exposed. Only the kata-paired SA is bound to a privileged SCC; the default SA stays restricted.
  ...
  * **Per-node user knobs** (all optional, mapped 1:1 from YAML to the Pod spec):
    ...
    * `workspace_size: <quantity>` controls the size of the in-memory workspace mounted at `/workspace`. The workspace is always a `Volume { emptyDir { medium: Memory, sizeLimit } }` so checkout, build artifacts, and step outputs live entirely in RAM. Unset values fall back to `kubernetes.default_workspace_size` (2Gi by default).
