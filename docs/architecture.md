...
* **KubeDispatcher:** The live `Dispatcher` implementation that actuates each pipeline node as a Kubernetes Job. There is a single node template — every node, regardless of workload, runs under the same shape:
  * **Sandboxed by default:** every Pod is submitted with `runtimeClassName: kata`. Kata-containers provides VM-level isolation between the workload and the host, which is what makes the rest of the model — arbitrary user code, optional `privileged: true`, `--network=host` builds — safe.
  * **Single namespace, single SA:** all Jobs land in `jefferies-jobs` under `serviceAccountName: jefferies-jobs-sa`. The previous `jefferies-builder` namespace, `pipelines-sa-userid-1000` SA, and `buildah-storage-config` ConfigMap are gone.
  ...
  * **Per-node user knobs** (all optional, mapped 1:1 from YAML to the Pod spec):
    * `privileged: true` sets `securityContext.privileged` on the user container — useful for inner builds (buildah, docker-in-docker) that need to mount overlay filesystems. The init container always stays unprivileged.
    * `cache_size: <quantity>` adds a `Volume { emptyDir { medium: Memory, sizeLimit } }` and mounts it at `/tmp/cache`. The user is responsible for pointing tools (`CARGO_HOME`, `PIP_CACHE_DIR`, `npm config set cache`, etc.) at that path.
    * `workspace_size: <quantity>` controls the size of the in-memory workspace mounted at `/workspace`. The workspace is always a `Volume { emptyDir { medium: Memory, sizeLimit } }` so checkout, build artifacts, and step outputs live entirely in the kata guest's RAM. Unset values fall back to `kubernetes.default_workspace_size` (2Gi by default).
    * `cpu` / `memory` are written to **both** `requests` and `limits` for deterministic sizing. Unset values fall back to `kubernetes.default_cpu` / `kubernetes.default_memory` (1 vCPU / 2Gi by default — the kata default). Kata pods run inside a fixed-size guest VM, so over-committing is meaningless; matching requests==limits keeps the scheduler honest.
  ...
  * The node image, Tube image, target namespace, service account, runtime class, and default CPU/memory are all user-configurable in `[kubernetes]` of the TOML config.
